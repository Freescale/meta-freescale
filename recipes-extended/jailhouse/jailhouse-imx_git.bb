...
# kernel-module-${KERNEL_VERSION} is generated by the kernel-module-split class
# (inherit module, above); declare it so its INSANE_SKIP is recognised by
# standalone lint. Scope the pattern to this recipe's own module only: a broad
# "^kernel-module-.*" makes jailhouse-imx a candidate provider for every
# kernel-module-* virtual (e.g. iptables' RRECOMMENDS kernel-module-x-tables),
# and since providers are sorted alphabetically it is picked ahead of linux-imx,
# pulling jailhouse-imx into the task graph of unrelated recipes.
